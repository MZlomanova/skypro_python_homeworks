...
def test_fill_form(chrome_browser: WebDriver):
    ...
    WebDriverWait(chrome_browser, 50, 3).until(EC.element_to_be_clickable((By.TAG_NAME, "button"))).click()
    sleep(3)
    
    assert "alert-danger" in chrome_browser.find_element(By.ID, "zip-code").get_attribute("class")
